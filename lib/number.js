...
exports.liNum = function(li) {
  const container = li.get('ancestor::*[self::ol or self::dl or self::ul][1]');

  // ol always has @start.  ul and dl don't.
  const start = parseInt(att(container, 'start')) || 1;
  const num = start + li.get('count(preceding-sibling::' + li.name() + ')')
  switch (att(container, 'type')) {
    case 'a':
      // TODO: lists longer than 26.
      return String.fromCodePoint('a'.charCodeAt(0) + num - 1);
    case 'A':
      // TODO: lists longer than 26.
      return String.fromCodePoint('A'.charCodeAt(0) + num - 1);
    case 'i':
